...
class Sca (Utils, Roc):
	...
	# set the channel
        ...
        self.wr_add_data = CRUADD['add_gbt_sca_wr_data']
        self.wr_add_cmd = CRUADD['add_gbt_sca_wr_cmd']
        self.wr_add_ctr = CRUADD['add_gbt_sca_wr_ctr']

        self.rd_add_data = CRUADD['add_gbt_sca_rd_data']
        self.rd_add_cmd = CRUADD['add_gbt_sca_rd_cmd']
        self.rd_add_ctr = CRUADD['add_gbt_sca_rd_ctr']
        self.rd_add_mon = CRUADD['add_gbt_sca_rd_mon']
...
#            self.cmd=np.bitwise_and(cmd, 0xff00ffff, casting='unsafe')
...
#        self.wr(0x14010091, 0x00000001, 1)
    ...
    def MFTEN_execute_cmd(self, debug = None, mat = None):
        ...
        for i in range(len(mat)): # loop to send the previously saved sequence (mat) to execution
                    sca_int = int(mat[i][0]) 
                    data_int = int(mat[i][1])
                    trid = None
                    ret = 1
                    while(ret == 1) :
                        self.wr(sca_int, data_int, 1, trid, debug)
                        time.sleep(mat[i][2])
                        ret, dat = self.rd(debug)
